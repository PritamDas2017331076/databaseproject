...
const Patientupdate = (props) => {
    ...
    const [hr, setHr] = useState(props.location.state.hr)
    const [min, setMin] = useState(props.location.state.min)
    const [day, setDay] = useState(props.location.state.day)
    const [month, setMonth] = useState(props.location.state.month)
    ...
    const onSubmit = (e) => {
        ...
        axios.patch(`http://localhost:5000/patients/${id}`,patientDetails)
         .then(res => {
